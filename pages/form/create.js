...
export default function Create() {
  ...
  const submit = async () => {
    ...
    await axios.post("http://54.151.56.35:8000/createform", {
      uuid,
      formTitle,
      formDesc,
    });

    await axios.post("http://54.151.56.35:8000/createquestions", {
      formUuid: uuid,
      questions,
    });
    ...
    optionArrayArray.map( async (options, index) => {
      ...
      const formUuid = uuid
      await axios.post(("http://54.151.56.35:8000/createoptions"), {qUuid: qUuid, formUuid: formUuid, options})
    });
